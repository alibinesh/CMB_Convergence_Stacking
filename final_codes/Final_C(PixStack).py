...
#pix_size = 0.5
#
#D6apodizing_window = cosine_window(act_map_d6)
#D51apodizing_window = cosine_window(act_map_d5_1)
#D52apodizing_window = cosine_window(act_map_d5_2)
#
#act_map_d6.data[:] = D6apodizing_window * act_map_d6.data[:]
#act_map_d5_1.data[:] = D51apodizing_window * act_map_d5_1.data[:]
#act_map_d5_2.data[:] = D52apodizing_window * act_map_d5_2.data[:]
#
#D6filt = make_2d_filter(act_map_d6,pix_size,ell_dd,filt_min, filt_max)
#D51filt = make_2d_filter(act_map_d5_1,pix_size,ell_dd,filt_min, filt_max)
#D52filt = make_2d_filter(act_map_d5_2,pix_size,ell_dd,filt_min, filt_max)
#
#act_map_d6 = generate_filt_map(act_map_d6, D6filt)
#act_map_d5_1 = generate_filt_map(act_map_d5_1, D51filt)
#act_map_d5_2 = generate_filt_map(act_map_d5_2, D52filt)
#
#act_map_d6.plot()
#act_map_d5_1.plot()
#act_map_d5_2.plot()
...
D6RA = -1*(gal_RA_new - 40.000000) * 60.0 * (1.0/0.501095)
D6Dec = (gal_dec_new + 7.799062) * 60.0 * (1.0/0.498239)
...
stacked_map = (D6_smap + D51_smap + D52_smap)
total_counter = D6_Counter + D51_Counter + D52_Counter + 3.0
stacked_map = stacked_map/total_counter

plt.figure(4)
from mpl_toolkits.axes_grid1 import make_axes_locatable
print("map mean:",np.mean(stacked_map),"map rms:",np.std(stacked_map))
plt.figure(figsize=[7,7])
im = plt.imshow(stacked_map, origin='lower',cmap=cm.jet)
ax=plt.gca()
divider = make_axes_locatable(ax)
#    	cax = divider.append_axes("right", size="5%", pad=0.05)
cbar = plt.colorbar(im)
